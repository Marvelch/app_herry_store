...
<div id="print">
    ...
    <div class="content">
        <div class="container-fluid">
            <div class="invoice p-3 mb-3">
                <div class="row">
                    <div class="col-12">
                        <h4> Herry Store <small class="float-right">Date:
                                {{ date_format($sale->created_at, 'd-M-Y') }}</small> <br />
                            <small>Pelayanan dan kualitas terbaik</small>
                        </h4>
                        <hr>
                    </div>
                </div>
                <div class="row invoice-info">
                    <div class="col-sm-4 invoice-col">Customer: <address> <strong>{{ $sale->client->name }}</strong>
                            {{ $sale->client->rfc }}<br> Address: {{ $sale->client->address }}<br> Phone:
                            {{ $sale->client->phone }}<br> Email: {{$sale->client->email}} </address>
                    </div>
    ...
    <p class="text-center"> <input type="button" class="btn text-center rounded-pill btn-light" value="Receipt Print"
            onclick="printPage('print');"></input> / For invoice print just presh ctrl + P</p>
</div> @endsection